...
def gen_epochs(num_steps,batch_size,epochs):
...
def train(Graph,num_epochs=-1,num_steps=50,batch_size=32,verbose=True,save=True):
	...
	with tf.Session() as sess:
		...
		training_losses = []
		for idx,epoch in enumerate(gen_epochs(num_steps,batch_size,num_epochs)):
			training_loss=0
			steps=0
			X=epoch[0].eval()
			Y=epoch[1].eval()
...
restoration_path=train(g,num_epochs=1,num_steps=200,batch_size=32)
...
# txt=generate_characters(g,text_length=1500)
print "".join(txt)
